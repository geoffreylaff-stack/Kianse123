...
export function parseInstrumentation(text) {
  ...
  for (const rawSegment of splitSegments(normalised)) {
    ...
    for (const [key, re] of PATTERNS) {
      ...
      let n = leadingCount(head, m.index);
      if (n === null) {
        // No explicit number: trust plurality, but flag it as an estimate.
        const isPlural = /(?:oboes|horns|hautbois|heckelphones|musettes|oboi|cors)\b/i.test(m[0]);
        n = isPlural ? 2 : 1;
        if (isPlural) uncertain.push(key);
      }
    ...
    for (const aside of asides) {
      ...
      for (const [key, re] of PATTERNS) {
        ...
        if (!primaryKey) counts[key] = counts[key] || 0;
        break;
      }
    }
  }
  ...
  return { counts, doublings, present, uncertain: [...new Set(uncertain)], total };
}
