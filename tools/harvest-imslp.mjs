#!/usr/bin/env node
/**
 * Harvests oboe / English horn repertoire from IMSLP into data/imslp.json.
 *
 ...
 *            "For 2 oboes, english horn, bassoon". Cheap and precise.
 *
...
const UA = 'OboeRepertoireFinder/1.0 (static site build step; contact via repo issues)';
...
async function passA() {
  ...
  for (;;) {
    ...
    for (const c of cats) {
      ...
      if (!mentionsOboeFamily(name)) continue;
  ...
  process.stderr.write(`  ${matching.length} oboe-family categories\n`);
...
function buildRecords(categoryHits, instrumentationByPage) {
  ...
  for (const rec of byPage.values()) {
    ...
    const alternates = rec.candidates.slice(1)
      ...
      .slice(0, 4)
      .map((c) => ({ scoring: formatOboeScoring(c.parsed), full: c.raw, arrangement: c.arrangement }));
    ...
    records.push({
      ...
      scoring: formatOboeScoring(best.parsed),
...
const featureCategories = ['Scores featuring the oboe', 'Scores featuring the english horn'];
