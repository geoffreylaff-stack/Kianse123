...
const UA = 'OboeRepertoireFinder/1.1 (static site build step; contact via repo issues)';
...
const COMPOSERS = [
  ...
  'Charles Ives', 'Howard Hanson', 'Walter Piston', 'John Adams', 'Philip Glass',
...
/**
 * Wikipedia writes instrumentation either as prose ("scored for 2 oboes, cor
 * anglais…") or one instrument per line (":4 oboes"). Flatten both to a single
...
// ── Harvest ───────────────────────────────────────────────────────────────────
const records = [];
const stats = { articles: 0, withSection: 0, withOboe: 0, skipped: 0 };
